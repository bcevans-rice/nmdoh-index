<template>
  <!-- Purple Header -->
  <section class="xl:px-36 lg:px-20 md:px-12 px-12 border-b border-gray-200 bg-nmdoh-purple pt-12 bg-header-background bg-no-repeat bg-center bg-cover">
    ...
    <p class="text-base text-left font-light text-white pb-4">
      ...
      <p>You may select one, all, or none of the areas listed under each field. You may also use the search bar below to find a specific term(s).</p>
    ...
    <div>
      ...
      <!--<TransitionRoot as="template" :show="mobileFiltersOpen">
        <Dialog as="div" class="relative z-40 lg:hidden" @close="mobileFiltersOpen = false">
          <TransitionChild ref="modal-bg" as="template" enter="transition-opacity ease-linear duration-300" enter-from="opacity-0" enter-to="opacity-100" leave="transition-opacity ease-linear duration-300" leave-from="opacity-100" leave-to="opacity-0">
            <div class="fixed inset-0 bg-black bg-opacity-25" />
          </TransitionChild>

          <div class="fixed inset-0 z-40 flex">
            <TransitionChild ref="modal-window" as="template" enter="transition ease-in-out duration-300 transform" enter-from="translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="translate-x-full">
              <DialogPanel class="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-6 shadow-xl">
                <div class="flex items-center justify-between px-4">
                  ...
                  <button type="button" class="-mr-2 flex h-10 w-10 items-center justify-center p-2 text-gray-400 hover:text-gray-600" @click="mobileFiltersOpen = false">
                  ...
                  <Disclosure as="div" v-for="section in filters" :key="section.name" class="border-t border-gray-200 pb-4 pt-4" v-slot="{ open }">
                    <fieldset>
                      <legend class="w-full px-2">
                        <DisclosureButton class="flex w-full items-center justify-between p-2 text-gray-400 hover:text-gray-600">
                      ...
                      <DisclosurePanel class="px-4 pb-2 pt-4">
                        <div class="space-y-6">
                          <div v-for="(option, optionIdx) in section.options" :key="option" class="flex items-center">
                            <input :id="`${section.id}-${option}-mobile`" :value="option" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-nmdoh-purple focus:ring-nmdoh-purple" />
                            <label :for="`${section.id}-${option}-mobile`" class="ml-3 text-sm text-gray-600">{{ section.options[optionIdx] }}</label>
                          </div>
                        </div>
                      </DisclosurePanel>
                    </fieldset>
